...
static int diasom_rk3568_evb_fixup(struct device_node *root, void *unused)
{
	...
	const struct cameras cameras[] = {
		...
		{ "camera8", diasom_rk3568_sony_imx662_detect },
		{ }
	};
	...
	if (!diasom_rk3568_probe_i2c(adapter, 0x18)) {
		pr_info("Camera AR0234 detected.\n");
		of_register_set_status_fixup("camera7", true);
	}
...
static int __init diasom_rk3568_init(void)
{
	...
	efuse = of_find_node_by_name_address(NULL, "efuse@fe38c000");

	if (efuse && !of_device_ensure_probed(efuse)) {
		struct device_node *cpuver =
			of_find_node_by_name_address(NULL, "cpu-info");

		if (cpuver) {
			char *val = nvmem_cell_get_and_read(cpuver, "cpu-version",
							    sizeof(char));
			if (val) {
				ver = *val;
				free(val);
